...
async function getIsochroneCoordinates(
  ...
  const urlMapbox = `${HTTPS_PROTOCOL}api.mapbox.com/isochrone/v1/mapbox/${meanOfTransport}/${coordinates.longitude},${coordinates.latitude}?contours_minutes=${minutes}&polygons=true&access_token=${API_TOKEN_MAPBOX}`;

  return await axios.get(urlMapbox).catch(error => {
    console.error(error);
  });
}

function getIsochronesCoordinatesForCylingOrWalking(
  locations: any[],
  travelTime: Number,
  meanOfTransport: String,
): Promise {
  ...
  for (const index in locations) {
    if (locations[index].GPSPosition.latitude !== null) {
      isochronesCoordinatesPromises.push(
        getIsochroneCoordinates(
          travelTime,
          locations[index].GPSPosition,
          meanOfTransport,
        ),
      );
    }
    ...
    .then(apiResponses => {
      ...
      for (const apiResponse of apiResponses) {
        newIsochronesCoordinates.push({
          ...
          reformattedCoordinates: reformatCoordinates(
            JSON.parse(
              JSON.stringify(apiResponse.data.features[0].geometry.coordinates),
            ),
          ), // useful for drawing multiPolygons
        });
      }
