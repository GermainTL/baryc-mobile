...
async function getIsochroneCoordinates(minutes: Number, coordinates: Object) {
  ...
  };${coordinates.latitude}&min_duration=0&max_duration=${minutes * 60}`;
  return await axios.get(urlNavitia, {
    headers: {
...
function getIsochronesCoordinates(
  locations: any[],
  travelTime: Number,
): Promise {
  ...
  for (const index in locations) {
    if (locations[index].GPSPosition.latitude !== null) {
      isochronesCoordinatesPromises.push(
        getIsochroneCoordinates(travelTime, locations[index].GPSPosition),
      );
    }
  }
      ...
      for (const apiResponse of apiResponses) {
        newIsochronesCoordinates.push({
          ...
          reformattedCoordinates: reformatCoordinates(
            JSON.parse(
              JSON.stringify(
                apiResponse.data.isochrones[0].geojson.coordinates,
              ),
            ),
